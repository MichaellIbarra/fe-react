...
const userInstitutionApiClient = axios.create({
  baseURL: 'https://lab.vallegrande.edu.pe/school/gateway/api/v1/user-institution', // URL del gateway
  headers: {
...
class UserInstitutionService {
  ...
  async deleteAllRelations(userId) {
    try {
      const response = await userInstitutionApiClient.delete(`/users/${userId}/institutions`);
      return response.data;
    } catch (error) {
  ...
  async changeRelationStatus(userId, status) {
    try {
      const response = await userInstitutionApiClient.patch(`/users/${userId}/status`, null, {
        params: { status }
      });
      return response.data;
    } catch (error) {
  ...
  formatUserDataForSubmit(userData) {
    return {
      ...
      role: userData.role,
      isActive: userData.status === 'A' || userData.isActive !== undefined ? userData.isActive : true
    };
  }
